...
def main() :
	yesterday = setDate()
	NBAresults = parseResults (yesterday)
	sendMail(NBAresults)
...
def parseResults (paramDay) :
	#	yesterday = setDate() --------- TO BE EECUTED IN MAIN
	results = []  # list containing pure results
	resultsFormatted = [] # list containing formatted results, with HTML tags

	#SET URLs
	resultsPageURL="http://www.basketball-reference.com/boxscores/?month=" + str(paramDay.month) + "&day=" + str(paramDay.day) + "&year=" + str(paramDay.year)
	...
	#again, removing some unWanted code
